...
function App() {
  ...
  React.useEffect(() => {
    (async () => {
      ...
      const hook = useAIProvider({ llm: { model: 'Xenova/gpt2', device: 'wasm', dtype: 'fp32', maxTokens: 10 }, autoLoad: true });
  ...
  return (
    React.createElement('div', null,
      ...
      error ? React.createElement('div', null, `Error: ${error.message}`) : null
    )
  );
}
